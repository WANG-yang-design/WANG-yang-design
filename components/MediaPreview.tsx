        ...
        <div className="flex-1 flex items-center justify-center bg-black relative overflow-hidden group">
          ...
          {category === FileCategory.VIDEO && (
            <div className="relative w-full h-full flex items-center justify-center">
              <video 
                ...
                controls={false} // Using custom controls if desired, but native is more robust for simple usage. Let's add overlay controls.
          ...
          {category === FileCategory.AUDIO && (
            <div className="flex flex-col items-center gap-8 p-12">
               ...
               <audio ref={audioRef} src={sourceUrl} onEnded={() => setIsPlaying(false)} />
            </div>
          )}
        ...
        <div className="w-full md:w-80 bg-surfaceHighlight border-l border-white/5 flex flex-col">
           ...
           {(category === FileCategory.VIDEO || category === FileCategory.AUDIO) && (
             <div className="p-6 flex flex-col gap-6">
                ...
                <div className="flex items-center justify-center gap-4">
                  <button 
                    onClick={handleTogglePlay}
                    className="w-14 h-14 rounded-full bg-white text-black flex items-center justify-center hover:scale-105 active:scale-95 transition-all"
                  >
                   ...
                   <div className="grid grid-cols-4 gap-2">
                      {[0.5, 1.0, 1.5, 2.0].map(rate => (
                        <button
                          ...
                          className={`py-1.5 rounded text-xs font-medium transition-colors ${playbackRate === rate ? 'bg-primary text-white' : 'bg-white/5 text-zinc-400 hover:bg-white/10'}`}
                        >
                          {rate}x
                        </button>
                      ))}
           ...
           <div className="mt-auto p-6 border-t border-white/5 space-y-3">
             <a 
               href={sourceUrl}
               download
               className="flex items-center justify-center gap-2 w-full py-3 bg-zinc-700 hover:bg-zinc-600 text-white rounded-lg text-sm font-medium transition-colors"
             >
               <Download className="w-4 h-4" />
               Download File
             </a>
